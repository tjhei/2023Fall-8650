...
void step_some_basics()
{
    int i = 4;

    ++i;
    std::cout << "printing things is a bit more complicated, but here is i=" << i << std::endl;
    ...
    std::vector<int> values = {1, 2, 3}; // a dynamic array containing ints, this is a template class
    values.push_back(4);
    int sum = std::accumulate(values.begin(), values.end(), 0);

    std::cout << "a vector with " << values.size() << " elements and sum = " << sum << " and entries:" << std::endl;
...
void f1(int x, std::vector<int> values, MyQueue queue)
{
    // all three arguments are copied
}

void f2(int &x, const std::vector<int> &values, const MyQueue &queue)
{
    // Here none of the arguments are copied. A const references can not be modified,
    // but a "non-const" one can. So, by convention, x is an "in-out" parameter.
    x = values[0];
}

void step_value_semantics()
{
    // By default, types are treated as values for function arguments and assignments
    // and as such, are copied. This includes simple types (int, float, double, bool, ...),
    // the standard library (std::vector, std::map, ...), and your own types.
    int x = 1;
    ...
    // Exercise:
    // - Write a function that given a vector of ints returns the smallest element.
}
